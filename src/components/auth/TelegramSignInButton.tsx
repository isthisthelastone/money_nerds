...
export function TelegramSignInButton({ returnTo = "/" }: { returnTo?: string }) {
  ...
  useEffect(() => {
    // Discard the UI hint left by the withdrawn native-SDK handoff. Never reopen
    // a cached tg:// link after this recovery release.
    try { window.sessionStorage.removeItem("mn_telegram_pending_v1"); } catch { /* Storage can be unavailable. */ }
  }, []);
